...
change_ssh_port() {
    ...
    local new_port
    while true; do
        read -p "Введите новый SSH порт (или 0 для выхода): " -r new_port
...
change_user_password() {
    ...
    if [[ ${#users[@]} -eq 1 ]]; then
        log_info "Найден только пользователь root"
    fi
...
# Главное меню SSH модуля с визуальным статусом
configure_ssh_security() {
    while true; do
        clear
        echo -e "${BLUE}╔══════════════════════════════════════╗${NC}"
        echo -e "${BLUE}║      SSH Security & Keys             ║${NC}"
        echo -e "${BLUE}╚══════════════════════════════════════╝${NC}"
        echo
        
        # Получаем текущие настройки SSH
        ...
        case $password_auth in
            no) echo -e "  • Аутентификация по ключу: ${GREEN}✅ Включена${NC}" ;;
            *) echo -e "  • Аутентификация по ключу: ${YELLOW}⚠️ Включена${NC}" ;;
        esac
        
        case $password_auth in
            no) echo -e "  • Парольная аутентификация: ${GREEN}❌ Отключена${NC}" ;;
            *) echo -e "  • Парольная аутентификация: ${RED}✅ Включена${NC}" ;;
        esac
        
        case $root_login in
            no) echo -e "  • Root доступ: ${GREEN}❌ Запрещён${NC}" ;;
            prohibit-password) echo -e "  • Root доступ: ${YELLOW}🔑 Только по ключу${NC}" ;;
            *) echo -e "  • Root доступ: ${RED}✅ Разрешён${NC}" ;;
        esac
        echo
        
        # Меню действий
        echo -e "${BLUE}🔑 УПРАВЛЕНИЕ КЛЮЧАМИ:${NC}"
        echo "1. 🔑 Управление SSH ключами (просмотр/добавление/удаление)"
        echo "2. 📤 Экспорт публичного ключа (в буфер)"
        echo
        echo
        echo -e "${BLUE}⚙️  НАСТРОЙКИ SSH:${NC}"
        echo "3. 🔢 Сменить SSH порт"
        echo "4. 🔒 Отключить парольную аутентификацию"
        echo "5. 🚫 Запретить root доступ"
        echo "6. 🔐 Изменить пароль пользователя"
        echo
        echo -e "${BLUE}💾 РЕЗЕРВНОЕ КОПИРОВАНИЕ:${NC}"
        echo "7. 💿 Создать бекап конфигурации"
        echo "8. ♻️  Восстановить из бекапа"
        echo
        ...
        case $choice in
            ...
            2) 
                ...
                if [[ -f /root/.ssh/id_rsa.pub ]]; then
                    cat /root/.ssh/id_rsa.pub | xclip -selection clipboard 2>/dev/null || \
                    cat /root/.ssh/id_rsa.pub | pbcopy 2>/dev/null || \
                    cat /root/.ssh/id_rsa.pub | wl-copy 2>/dev/null || {
                        log_warning "Не удалось скопировать в буфер обмена"
                        echo
                        cat /root/.ssh/id_rsa.pub
                    }
                    log_success "Публичный ключ скопирован в буфер обмена"
                else
                    log_error "Публичный ключ не найден"
                fi
                ;;
            3) change_ssh_port ;;
