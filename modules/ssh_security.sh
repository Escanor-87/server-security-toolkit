...
generate_ssh_key() {
    ...
    if [[ ${#existing_keys[@]} -gt 0 ]]; then
        echo
        log_info "Найдены существующие ключи: ${existing_keys[*]}"
        echo
    fi
    ...
    echo "0. 🔙 Назад в меню"
    echo
    read -p "Выберите тип [0-2]: " -n 1 -r key_type
    echo
    ...
    # Проверяем существующий ключ выбранного типа
    if [[ -f "$key_file" ]]; then
        echo
        log_warning "Ключ уже существует: $key_file"
        read -p "Перезаписать? (Enter = да, 0 = отмена): " -r
        if [[ "$REPLY" == "0" ]]; then
            log_info "Генерация отменена"
            return 0
        fi
...
configure_ssh_security() {
    while true; do
        ...
        echo "3. 🔒 Отключить парольную аутентификацию"
        echo "4. 🚫 Запретить root доступ"
        echo "5. 🔐 Изменить пароль пользователя"
        echo "6. 💿 Создать бекап конфигурации"
        echo "7. ♻️  Восстановить из бекапа"
        echo
