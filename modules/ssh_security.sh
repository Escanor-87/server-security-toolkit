...
configure_ssh_security() {
    while true; do
        ...
        case $password_auth in
            no) echo -e "  • Ключевая аутентификация: ${GREEN}✅ Включена${NC}" ;;
            *) echo -e "  • Ключевая аутентификация: ${YELLOW}⚠️ Включена${NC}" ;;
        esac
        ...
        echo "  • Активных ключей: $key_count"
        echo
        
        # Меню действий
        echo -e "${BLUE}🔑 УПРАВЛЕНИЕ КЛЮЧАМИ:${NC}"
        echo "1. 📋 Показать все ключи"
        echo "2. ➕ Добавить новый ключ"
        echo "3. 📤 Экспорт публичного ключа (в буфер)"
        echo
        echo -e "${BLUE}⚙️  НАСТРОЙКИ SSH:${NC}"
        echo "4. 🔢 Сменить SSH порт"
        echo "5. 🔒 Отключить парольную аутентификацию"
        echo "6. 🚫 Запретить root доступ"
        echo "7. 🔐 Изменить пароль пользователя"
        echo
        echo -e "${BLUE}💾 РЕЗЕРВНОЕ КОПИРОВАНИЕ:${NC}"
        echo "8. 💿 Создать бекап конфигурации"
        echo "9. ♻️  Восстановить из бекапа"
        echo
